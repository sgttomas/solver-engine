...
### 0.1 Document Authority Order

When documents appear to conflict, authority flows in this order:

```
Design Intent (Why²)        ← Highest authority: first principles
       ↓
Architectural Contract (Why) ← Invariants and correctness rules
       ↓
Technical Specification (What) ← Implementation mechanics
       ↓
Development Directive (How)  ← Build plan and sequencing
       ↓
README (Orientation)         ← Onboarding and operations
```

**Resolution rule:** If two documents appear to disagree, the higher-authority document controls. The lower-authority document must be corrected to align.
...
□ SHA-256 hash computed for each document
□ Timestamp recorded
...
2. Create `FREEZE-RECORD.md` artifact containing:
   ...
   - Document manifest (filename + version + SHA-256 for each)
   - Timestamp
